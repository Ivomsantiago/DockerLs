...
### Added
- A provider-neutral CI integration boundary with deterministic detection for
  Azure DevOps, GitHub Actions, GitLab CI, Jenkins, Bitbucket Pipelines,
  CircleCI, generic CI, and local execution.
- `dockerls analyze --ci`, which emits stable JSON on stdout without a spinner,
  Rich markup, colors, or terminal escape sequences.
- Reusable pipeline examples for Azure DevOps, GitHub Actions, GitLab CI,
  Jenkins, and generic shell pipelines.

### Security
- Provider logging commands redact credentials, collapse untrusted multiline
  text, and escape Azure/GitHub control syntax before emission.
- Connectors are presentation-only: they cannot modify the analysis result,
  policy, confidence, or verdict.

### Changed
- Machine-readable `analyze` output is written directly to stdout instead of
  passing through Rich, preventing wrapping or highlighting from corrupting
  JSON/SARIF documents.
