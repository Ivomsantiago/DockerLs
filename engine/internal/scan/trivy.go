...
// Scanner mede uma imagem com o Trivy.
type Scanner struct {
	path           string
...
func (s *Scanner) Scan(ctx context.Context, reference, cacheDir string) protocol.Result {
	...
	if !referencePattern.MatchString(reference) {
		return failure(reference, timestamp, protocol.StatusError, protocol.KindInvalidOutput,
	...
	run := runCapture(ctx, argv, s.timeout, s.maxOutputBytes)

	switch {
	case run.timedOut:
		return failure(reference, timestamp, protocol.StatusTimeout, protocol.KindTimeout,
			fmt.Sprintf("Scan exceeded %gs timeout", s.timeout.Seconds()))

	case errors.Is(run.err, ErrOutputTooLarge):
		return failure(reference, timestamp, protocol.StatusError, protocol.KindInvalidOutput,
			ErrOutputTooLarge.Error())

	case run.err != nil:
		if errors.Is(run.err, os.ErrNotExist) || errors.Is(run.err, os.ErrPermission) {
			return failure(reference, timestamp, protocol.StatusError, protocol.KindScannerMissing,
				run.err.Error())
		}
		return failure(reference, timestamp, protocol.StatusError,
	...
	case len(run.stdout) == 0:
		return failure(reference, timestamp, protocol.StatusError, protocol.KindInvalidOutput,
			"Trivy produced no output")
	}

	result, err := ParseTrivy(reference, run.stdout, timestamp)
	if err != nil {
		...
		return failure(reference, timestamp, protocol.StatusError, protocol.KindInvalidOutput, detail)
	}
...
func failure(reference, timestamp, status, kind, message string) protocol.Result {
